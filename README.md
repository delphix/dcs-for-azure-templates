...
repository, in any number of ways. In all the following examples, the directory you use for each pipeline is
`<template_dir>`:
* On Windows:
...
A metadata store (created on Azure SQL) will consist of a few tables, a series of scripts for defining the tables required by the metadata
...
the required version for templates to work - each template will contain a README that will
indicate what the minimum version of the metadata store is required. We aim to make it so that all migrations
    ...
    1. Automatically, via the discovery pipeline. The data in this table is populated in two stages.
       ...
       the `discovered_ruleset` table with as much of the following information as possible:
          * `dataset` (static) - determines which type of data this is, it will be something like `SNOWFLAKE`, but
             will be representative of the datasource; this value will be statically populated in the appropriate
          ...
          * `specified_database` (parameter) - the top-level identifier for the dataset, in the case of things
             like AzureSQL and Snowflake this will be the `database`, for things like Databricks it will mean
             the `catalog`
          ...
          * `identified_column` (determined) - the fourth-level identifier for the dataset, in the case of things
             ...
             data sources that have an analogous concept, but with a different name
          * `identified_column_type` (determined) - either the exact data type or a slightly modified version provided by the information schema, this modification is used in
       ...
       2. Collect few rows of data from the specified table and column combination and perform data discovery to determine if the
          data is likely to be sensitive. This is done by calling the `profileByColumn` endpoint in the discovery component of
          DCS for Azure services, collecting the results of profiling, and persisting them to the `discovered_ruleset`
          metadata store table, populating for key `(dataset, specified_database, specified_schema, identified_table,
          ...
          * `discovery_complete` (determined) - bit representing whether the data in this column has had data discovery
            performed on it
          * `latest_event` (determined) - unique identifier that is used to refer to events in event logging table which
...
* `adf_type_mapping` - This table is used to track how data should be treated as it flows through masking pipeline in
   Azure Data Factory, where applicable. The values in this table are inserted by the migration scripts.

## Building A Ruleset

For data to be masked, there must be a mapping in the `adf_data_mapping` table, and at least one row having `assigned_algorithm`
value assigned in the `discovered_ruleset` table. Note that the `profiled_algorithm` that is populated as a result of the profiling pipeline should
...
* In Pipeline Parameters:
  ...
  * `P_FAIL_ON_NONCONFORMANT_DATA` - Default: `true` - This controls the behavior of non-conformant data errors that may
    ...
    fail the pipeline, the non-conformant data (unmasked) can land in the sink instead by setting this to `false`; in
  ...
  * `P_REAPPLY_MAPPING` - Default: `true` - This controls whether to attempt to re-apply the source to sink mappings
    ...
    masked.
  * `P_TRUNCATE_SINK_BEFORE_WRITE` - Default: `true` - This controls whether the sink should be cleared off data before
...
* In `assigned_algorithm`:
  ...
  * If masking is to be applied, the value can be an available algorithm found in the web-UI.
  * If the data in this column changes based on the value in a different column, then the data that is assigned to
  ...
  * `date_format` - When a custom date format is required for a date algorithm the key `date_format` must be specified
    ...
    * The important thing here is that the JSON value in the `algorithm_metadata` column contains a `date_format` key at
      ...
      * As an example, if you have a column `transaction_date` in your `Snowflake` instance, and each time that column
        ...
        ```
  * `treat_as_string` - When a field has a type that is incompatible with a type required for masking (e.g. when a numeric
...
To apply different algorithms to the values of the same column based on data present in a different key column, it is important to set the `assigned_algorithm`
with conditions to both key column and target columns for the masking pipeline to work.

#### Defining the Key Conditions on the Key Column

The key column is the column whose value must be evaluated to determine which algorithm to apply on other target columns. The `assigned_algorithm`
...
The `alias` key defines the alias for the filter condition, this will be used when defining conditional algorithms on other columns. All
...
the query to determine the conditions will automatically build the conditions for the default condition by combining the negation of all other conditions with a logical `AND`. Also note that we support a shortcut for referencing the value
in the key column (`%` by default), so that more complex filter conditions can be built.

Consider a `key_column` with values `KEY_1`, `KEY_2`, `KEY_3`, `KEY_4`, `KEY_5`, and `KEY_6`. Consider a few different
alias keys `K1`, `K2`, `K3` that help us identify conditions on the value of the key_column at a particular row, and set a default behavior for everything else as well.

We can construct the value of `assigned_algorithm` for the `key_column` using a structure like the following:
```json
[
    {
        "alias": "K1",
...
What this means is that the table will need to be masked in 4 stages (one for each condition), `K1` will mask all rows
where the value in the `key_column` is exactly equal to `KEY_1`, `K2` will mask all rows where the value in the `key_column`
ends with `2`, `K3` will mask all rows where the value in the `key_column` starts with `KEY_3`, and `default` will mask
...
#### Defining the Conditional Algorithms on the Target Columns

Conditional algorithms on the target columns are defined by specifying the `key_column`, using the `key_column`'s aliases and assigning an
algorithm names in those conditions.

Considering the example above that describes how to specify the `key_column`'s aliases, let's suppose that column's name
...
Note that we're specifying the `key_column` and referring to the column by its name, we're specifying the conditions in which
we will apply masking, which are referred to by the `alias` defined in the `key_column`, and we specify the algorithms to
