...
* Select Tables That Require Masking. This is done by querying the metadata store. This will provide a list of tables
  ...
  * For Each Table To Mask
    * Lookup Masking Parameters - calls the generate_masking_parameters stored procedure to generate masking parameters
      for both conditional and non-conditional masking scenarios
    * Check if the table must be masked with a filter condition
      ...
      * If a filter needs to be applied:
        * Call the `dcsazure_ADLS_to_ADLS_parquet_filtered_mask_df` dataflow, passing in parameters as generated by the
          `Lookup Masking Parameters` activity and the filter as determined by the output of For Each Table To Mask
        * Update the mapped status based on the success of this dataflow, and fail accordingly
...
* `METADATA_MASKING_PARAMS_PROCEDURE_NAME` - This is the name of the stored procedure used to generate masking parameters
  for both conditional and non-conditional masking scenarios (default `generate_masking_parameters`).
* `COLUMN_WIDTH_ESTIMATE` - This is the estimated width of the columns when the actual width is not available from database
  schema to perform the calculation of optimal number of batches in unconditional masking (default `1000`).
