...
* `[R]` Derived Column - `ModifyNumberOfBatches` - Modifies the number of batches to be at least `1`
* `[S]` Parse - `ParseMetadata` - Parse the content from the `metadata` column that contains JSON, specifically
